...
import { hashBytecode, CONTRACT_DEPLOYER, CONTRACT_DEPLOYER_ADDRESS, EIP712_TX_TYPE } from './utils';
...
export class ContractFactory extends ethers.ContractFactory {
    ...
    override async deploy(...args: Array<any>): Promise<Contract> {
        ...
        const deployedAddresses = deployTxReceipt.logs.filter(
            (log) => log.topics[0] == utils.id('ContractDeployed(address,bytes32,address)')
        );
        ...
        const deployAddress = utils.getAddress(rawAddress);

        const contractWithCorrectAddress = new ethers.Contract(deployAddress, contract.interface, contract.signer);
